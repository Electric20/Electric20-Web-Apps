<?php
	...
	$results = mysql_query("select UNIX_TIMESTAMP(timeStamp)*1000 as time, tblLoadAgg.loadAvg from tblLoadAgg where hubId=7 and sensorId=1260 and timeStamp >(CURRENT_TIMESTAMP - interval 1 day) order by timeStamp desc;
	");
	$dataset1 = array();
	while ($row = mysql_fetch_assoc($results)) 
	{
			$dataset1[] = array($row['time'],$row['loadAvg']);	
    }	
?>
